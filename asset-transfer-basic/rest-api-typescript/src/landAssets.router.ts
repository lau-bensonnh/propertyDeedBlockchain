    ...
    try {
        ...
        if (propertyReferenceNumber !== '') {
            assets = assets.filter(
                (asset: any) =>
                    asset.propertyReferenceNumber === propertyReferenceNumber
            );
        }
        if (addressFilter !== '') {
            assets = assets.filter((asset: any) =>
                asset.propertyAddress.includes(addressFilter)
            );
        }
...
landAssetsRouter.options('/:assetId', async (req: Request, res: Response) => {
    const assetId = req.params.assetId;
